...
class SortTask(ST):
    ...
    async def _embysort(self,media):
        async with self.server.sem:
            try:
                ...
                elif isinstance(self.server,Jellyfinserver):
                    if media.ForcedSortName != (titlevalue[0] if titlevalue[0].isdigit() else titlevalue) or \
                    media.OriginalTitle != final:
                        await media.fetchitem()
                        media.data["OriginalTitle"] = final
                        media.data["ForcedSortName"] = titlevalue[0] if titlevalue[0].isdigit() else titlevalue
    ...
    async def run(self):
        ...
        try:
            ...
            for lb in sections:
                if isinstance(self.server,(Embyserver,Jellyfinserver)):
