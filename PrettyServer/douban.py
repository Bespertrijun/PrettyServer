...
class Douban(Util):
    ...
    async def query(self, path, method=None, headers=None, data=None, json=None, msg: str = None):
        async with self.sem:
            # 随机休眠 3-10 秒
            await asyncio.sleep(3 + int(time.time()) % 7)
            return await super().query(path, method, headers, data, json, msg)
...
async def main():
        ...
        resp = await douban.query(url, 'post',data=auth_data,headers=header)
        print(resp)
